...
const DataStatsOne: React.FC<dataStats> = () => {
  ...
  const [customUsersCount, setCustomUsersCount] = useState(0);
  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      try {
        ...
        // Filter invoices with type "customer"
        const customerInvoices = allInvoices.filter((invoice: any) => invoice.type === "customer");

        // Set the count of customer invoices
        setInvoicesCount(customerInvoices.length);

        // Set all invoices
        setInvoices(allInvoices);

        // Calculate total price for customer invoices only
        const total = customerInvoices.reduce(
          (acc: number, invoice: any) => {
            const price = Number(invoice.total) || 0; // Use `total` and default to 0 if undefined
            return acc + price;
          },
          0
        );

        // Format total without decimals and with a period as the thousands separator
        const formattedTotal = Math.round(total).toLocaleString('fr-FR').replace(/\s/g, '.');
  ...
  // Fetch users and count users with role "custom"
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        // Filter users with role "custom"
        const customUsers = allUsers.filter((user: any) => user.role === "custom");
        setCustomUsersCount(customUsers.length);
        setUsers(allUsers);
  ...
  // useEffect(() => {
  //   // Calculate total price based on fetched invoices
  ...
  //         // Parse the total amount from the invoice
  //         const invoiceTotal = parseFloat(invoice.total);

  //         // Check if the parsed value is a valid number
  //         if (!isNaN(invoiceTotal)) {
  //           // Add the invoice total to the total price
        ...
        >
          ...
          <div className="mt-6 flex items-end justify-between">
            <div>
              ...
              <span className="text-body-sm font-medium">Totale Factures</span>
            </div>
        ...
        >
          ...
          <div className="mt-6 flex items-end justify-between">
            <div>
              ...
              <span className="text-body-sm font-medium">Totale Revenue</span>
            </div>
        ...
        >
          ...
          <div className="mt-6 flex items-end justify-between">
            <div>
              ...
              <span className="text-body-sm font-medium">Totale Produits</span>
            </div>
          </div>
        </div>
        <div
        ...
        >
          ...
          <div className="mt-6 flex items-end justify-between">
            <div>
              ...
              <span className="text-body-sm font-medium">Client Fidele</span>
            </div>
          </div>
        </div>
      </div>
