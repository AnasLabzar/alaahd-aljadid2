...
// Define the structure for the series in revenueChart
interface Series {
    name: string;
    data: number[];
}
...
interface RevenueChart {
    series: Series[];
    options: any;  // You can type this more strictly if you know the structure
}


const Index = () => {
    const isRtl = useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl'; // Declare isRtl correctly
    ...
    }, [dispatch]); // Added dependency array to avoid warnings

    // Function to fetch invoice count based on time filter
    const fetchInvoiceCount = async (filter: string) => {
        ...
        try {
            ...
            const newSeries: Series[] = [
                {
                    name: 'Invoices',
                    data: Array(12).fill(response.data.count || 0) // Fills data for 12 months or weekly count
                }
            ];
    ...
    // Fetch invoice count when component mounts or when time filter changes
    useEffect(() => {
        fetchInvoiceCount(timeFilter);
    }, [timeFilter]);
                ...
                <div className="grid xl:grid-cols-3 gap-6 mb-6">
                    <div className="panel h-full xl:col-span-2">
                        <div className="flex items-center justify-between dark:text-white-light mb-5">
                            ...
                            <div className="dropdown">
                                ...
                                >
                                    <ul>
                                        <li>
                                            <button type="button" onClick={() => setTimeFilter('weekly')}>Weekly</button>
                                        </li>
                                        <li>
                                            <button type="button" onClick={() => setTimeFilter('monthly')}>Monthly</button>
                                        </li>
                                        <li>
                                            <button type="button" onClick={() => setTimeFilter('yearly')}>Yearly</button>
                                        </li>
                                    </ul>
                                </Dropdown>
                            </div>
                        ...
                        <p className="text-lg dark:text-white-light/90">
                            Total Invoices: <span className="text-primary ml-2">{invoiceCount}</span>
                        </p>
